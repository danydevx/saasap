...
const form = reactive({
  ...
  is_primary: location.value.is_primary || false,
  is_active: location.value.is_active || false,
})

const submit = () => {
  router.put(`/admin/businesses/${business.value.id}/locations/${location.value.id}`, form)
}
</script>
