          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <button v-if="!uploading && files.length > 0" type="button" class="btn btn-primary" @click="startUpload">
                <i class="bi bi-cloud-arrow-up me-1"></i> Subir {{ files.length }} imagen(es)
              </button>
            </div>
          </div>
        </div>
...
const addFiles = (newFiles) => {
  ...
  const validFiles = fileArray
    .filter(file => {
      ...
      if (!isValidType) {
        formError.value = 'Solo se permiten archivos JPG, PNG o WebP'
      } else if (!isValidSize) {
...
const startUpload = () => {
  ...
  router.post(`/member/businesses/${props.businessId}/properties/${props.propertyId}/images`, formData, {
    preserveScroll: true,
    onSuccess: () => {
      files.value = []
      closeModal()
...
const deleteImage = (imageId) => {
  ...
  router.delete(`/member/businesses/${props.businessId}/properties/${props.propertyId}/images/${imageId}`, {
    preserveScroll: true,
    onSuccess: () => {
      emit('updated')
    },
