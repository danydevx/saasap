...
class StorePropertyRequest extends FormRequest
{
    ...
    protected function addDynamicFieldRules(array $rules, ?int $propertyTypeId): array
    {
        ...
        $allowedSectionIds = [1, 2, 6];

        $fields = PropertyField::where('property_type_id', $propertyTypeId)
            ->where('is_active', true)
            ->whereHas('generalField', function ($query) use ($allowedSectionIds) {
                $query->whereIn('general_field_section_id', $allowedSectionIds);
            })
            ->get();
